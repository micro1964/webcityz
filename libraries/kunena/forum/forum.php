...
abstract class KunenaForum {
	...
	protected static function buildVersion() {
		if ('3.0.4' == '@' . 'kunenaversion' . '@') {
			...
			self::$version = strtoupper ( '3.0.4' );
		}
		self::$version_major = substr(self::$version, 0, 3);
		self::$version_date = ('2013-12-22' == '@' . 'kunenaversiondate' . '@') ? JFactory::getDate()->format('Y-m-d') : '2013-12-22';
		self::$version_name = ('Ukko' == '@' . 'kunenaversionname' . '@') ? 'Git Repository' : 'Ukko';
	}
}
