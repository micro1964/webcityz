	...
	<div class="itemList">
		...
		<div id="itemListLeading">
		 ...
		 <?php 
			 ...
			 $customSpans = '';
			 $icols = $this->params->get('num_leading_columns');
			 $spaces = 12; $cs = 0;
			 if (is_array($customSpans)) {
			 	$cs = count($customSpans);
			 }
	
			 $spanClass = floor($spaces / ($icols - $cs));
			 if ($spanClass == 0) $spanClass = 1;
                         
			 $key= ($key-0)+1;
			 $rowcount=( ((int)$key-1) %	(int) $icols) +1;
			 $row = $counter / $icols;
			 if ($rowcount==1) : ?>
			 <div class="leading-row leadingcols-<?php echo (int) $icols;?> <?php echo 'row-'.$row ; ?> row-fluid">
			 <?php endif; ?>
			...
			<div class="span<?php echo $spanClass;?>">
			 ...
			 <?php $counter++; ?>
			 <?php if (($rowcount == $icols) or ($counter ==$leadingcount)): ?>
				</div>
			<?php endif; ?>
			<?php endforeach; ?>
			<div class="clr"></div>
		...
		<div id="itemListSecondary">
		 ...
		 <?php 
			 ...
			 $customSpans = '';
			 $scols = $this->params->get('num_secondary_columns');
			 $spaces = 12; $cs = 0;
			 if (is_array($customSpans)) {
			 	$cs = count($customSpans);
			 }
	
			 $spanClass = floor($spaces / ($scols - $cs));
			 if ($spanClass == 0) $spanClass = 1;
                         
			 $key= ($key-0)+1;
			 $rowcount=( ((int)$key-1) %	(int) $scols) +1;
			 $row = $counter / $scols;
			 if ($rowcount==1) : ?>
			 <div class="primary-row primarycols-<?php echo (int) $scols;?> <?php echo 'row-'.$row ; ?> row-fluid">
			 <?php endif; ?>
			...
			<div class="span<?php echo $spanClass;?>">
			 ...
			 <?php $counter++; ?>
			 <?php if (($rowcount == $scols) or ($counter ==$secondarycount)): ?>
				</div>
			<?php endif; ?>
			<?php endforeach; ?>
			<div class="clr"></div>
