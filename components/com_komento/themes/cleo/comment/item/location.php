...
<span class="kmt-location"<?php if( $system->konfig->get( 'enable_schema' ) ) echo ' itemprop="contentLocation" itemscope itemtype="http://schema.org/Place"'; ?>>
	<?php if( $row->address && $row->latitude && $row->longitude ) { ?>
		<?php //echo JText::_( 'COM_KOMENTO_COMMENT_FROM' );?>
		<i></i><a href="http://maps.google.com/maps?z=15&amp;q=<?php echo $row->latitude;?>,<?php echo $row->longitude;?>" target="_blank"><?php echo $row->address;?></a>
	<?php } else { ?>
		<?php if( $row->address ) { ?>
