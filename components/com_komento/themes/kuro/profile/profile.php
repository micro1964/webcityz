...
<div id="section-kmt">
	...
	Komento.require().script('komento.profile', 'komento.language').done(function($) {
		Komento.options.element.profile = $('#kmt-profile-tabs').addController('Komento.Controller.Profile', {
	...
	<div id="kmt-profile-tabs">
		<ul class="kmt-profile-tab reset-ul float-li clearfix">
			<?php if( $system->konfig->get( 'profile_tab_comments' ) ) { ?>
			<li><a href="javascript:void(0);" class="navComments navs" tab="kmt-comments" func="loadComments" title="kmt-comments"><?php echo JText::_( 'COM_KOMENTO_PROFILE_TAB_COMMENTS' ); ?></a></li>
			<?php } ?>
		...
		<div id="kmt-comments" class="tabs"></div>
