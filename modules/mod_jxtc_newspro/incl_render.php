...
for($p=1;$p<=$pages;$p++) {
	...
	$mainareahtml .= '><div class="table'.$jxtc.' '.$pageclass.' page-'.$p.'">';
	...
	for ($r=1;$r<=$rows;$r++) {
		...
		$mainareahtml .= '<div class="'.$rowclass.' clearfix row-'.$r.'">';
		for ($c=1;$c<=$columns;$c++) {
			...
			if (!empty($item)) {
				...
				$mainareahtml .= '<div class="xtc-grid-'.$columns.' nsp-responsive '.$colclass.' col-'.$c.'">'.$itemhtml.'</div>';
				$index++;
			}
		}
		$mainareahtml .='</div>';
	}
	$mainareahtml .= '</div></div>';
}
$mainareahtml .= '</div>';
...
if ($realpages) {
	...
	$leftbuttonhtml = ($realpages > 1) ? $img_prev : '';
	$rightbuttonhtml = ($realpages > 1) ? $img_next : '';
